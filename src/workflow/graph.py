...
from src.agents.recon_agents import (
     web_recon_specialist,
     osint_analyst,
     vulnerability_analyst,
     subdomain_analyst,
     live_host_analyst,
     js_recon_analyst,
     parameter_discovery_analyst,
)
...
def run_web_recon(state: ReconState):
     logger.info(f"Starting Web Recon on {state['target']}")
     try:
          task = Task(
...
def run_osint(state: ReconState):
     logger.info(f"Starting OSINT on {state['target']}")
     try:
          task = Task(
...
def run_vulnerability_scan(state: ReconState):
     logger.info(f"Starting Vulnerability Scan on {state['target']}")
     try:
          task = Task(
...
def run_subdomain_recon(state: ReconState):
     logger.info(f"Starting Subdomain Recon on {state['target']}")
     try:
          task = Task(
...
def run_live_host(state: ReconState):
     logger.info(f"Starting Live Host Check on {state['target']}")
     try:
          task = Task(
...
def run_js_recon(state: ReconState):
     logger.info(f"Starting JS Recon on {state['target']}")
     try:
          task = Task(
...
def run_parameter_discovery(state: ReconState):
     logger.info(f"Starting Parameter Discovery on {state['target']}")
     try:
          task = Task(
...
workflow.add_edge(
     ["web_recon", "osint", "vuln_scan", "subdomain_recon", "live_host", "js_recon", "parameter_recon"],
     "compile_report",
)
